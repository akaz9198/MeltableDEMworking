...
class Cconfig
{
	...
	QList <Ccontact>  C;		/**< List of contact.*/
	QList <Ccontact>  CThread[NTHREADS]; /**< Temperaty contact lists for threads for MPI, YG */
	QList <Cparticle> P;		/**< List of particle.*/
...
//	int Number_P_in_G;
	...
	void set_radius(QList <double> &radius, int &Nf);/**< Create a list of radius with either a uniform distribution of fractale distribution. */
