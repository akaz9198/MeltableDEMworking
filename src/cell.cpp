...
void Ccell::predictor(double dt,double dt2_on_2)
{
	...
	if(normal_stress_control)//if the normal stress is controled
		{
			...
			L.x[1] += Yshift;
			Vdilat +=  Adilat*dt;
		if( Vdilat < -0.02*L.x[1]) Vdilat= -0.02*L.x[1]; //limit cell velocity
		else if(Vdilat >  0.02*L.x[1]) Vdilat=  0.02*L.x[1]; //limit cell velocity
	...
	if(stick_slip){
		double Host_Depth = 1e5;
		double Host_Shear_Modulus = 1000/2.0; // G=E/2(1+\nv)
//		double MassFactor = 1.0*L.x[1]/100.0;
		double MassFactor = 1.0/10.0; // mass surface density = \sigma_n /g
...
*/
	...
	{
		Ashear_p =  Ashear;
		Ashear 	 =  (shear_stress_ext-shear_stress_in)/mass;
