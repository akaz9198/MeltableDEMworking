...
void Cparticle::predictor(double dt,double dt2_on_2)
{
	...
	double Tm = 21;
	double Lm = 10.0*c;	// set to a small fraction of the total latent heat of the particle for a thin shell.
	double dl = 0.0;
	...
	if(L>0.0)
	{
		...
		RS = pow(RS, 1.0/3.0);
		dRS = -dl/4.0/PI/RHO/Lm/(RS*RS);
		// [To Do] for limiting rS
		if(RS<0.75*R && dRS<=0.0) {
			RS=0.75*R;dRS=0.0;}
	}
}
...
ofstream & operator<<(ofstream &file,Cparticle p)
{
	...
	file<<p.R<<"\t"<<p.m<<"\t"<<p.T<<"\t"<<p.AM_I_BOUNDARY<<"\t"<<p.L;//scalar
	file<<endl;	//new line
 	return file;
}
	...
	for(int k=0;k<my_box.size();k++)
		for(int i=0;i<my_box[k]->part.size();i++) 
			if(my_box[k]->part[i]->id ==id )	
				{my_box[k]->part.removeAt(i);break;}
