...
for(config.t=tstart;config.t<tend;config.t+=dt)   //start time loop
	{
		...
		{ 
			screen.next += 1;
			int nc=0, np=0;
			for(int ic=0;ic<config.C.size();ic++) if(config.C[ic].deltaNB >0) nc++;
			for(int ip=0;ip<config.P.size();ip++) {
				if(config.P[ip].RS < config.P[ip].R) np++;	}
		...
		file<<config.t<<"\t"<<config.cell.Xshift<<"\t"<<config.cell.L.x[1]<<"\t"<<config.cell.stress.x[1][1]<<"\t"<<config.cell.stress.x[1][0]<<"\t"
		<<config.parameter.average_temperature<<"\t"<<config.heat_in<<"\t"<<config.heat_out<<"\t"<<config.PN<<"\t"<<config.PS<<"\t"<<config.PT<<"\t"<<config.PR
		<<"\t"<<nc<<"\t"<<np<<endl; 
		file.close();
		}
	}
